...
def visualizer(queue: Queue, shutdown_event: Event):
    ...
    curves = [plot.plot(pen=pg.intColor(i)) for i in range(4)]

    buffer_size = 256 * 5  # 5 seconds of data at 256Hz
    data_buffers = [np.zeros(buffer_size) for _ in range(4)]

    channel_offsets = [100 * i for i in range(4)]  # Vertical offsets per channel
