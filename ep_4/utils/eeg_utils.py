...
def compute_relative_alpha(powers, fs=256, window_sec=2, alpha_band=(8, 13), beta_band=(14, 30)):
    ...
    for p in powers:
        ...
        beta_power = np.sum(p[beta_idx])
        alpha_power = np.sum(p[alpha_idx])
        relative_alpha.append(alpha_power / (alpha_power + beta_power))
        #relative_alpha.append(alpha_power / total_power)
    return np.array(relative_alpha)
    
    
    
def compute_relative_alpha_from_signal(signal, fs=256, alpha_band=(8, 13)):
    """
    Compute relative alpha power from a time-series (single channel).
    ...
    alpha_power = np.sum(power[alpha_idx])
    return alpha_power / total_power if total_power > 0 else 0.0